...
public class DarkSoulCutAction extends AbstractGameAction {
    ...
    public void update(){
        ...
        for(int i =0;i<3;i++) {
            addToTop(new DamageAction(this.target, this.info, this.effect));
        }
        ...
        if (this.target != null && this.target.currentHealth > 0) {
            if(AbstractDungeon.player.hasRelic("Boot") && (this.damage <5 && this.damage>0)){
                if (15 >= this.target.currentHealth + this.target.currentBlock && !this.target.halfDead && !this.target.hasPower("Minion")) {
                ...
                if (this.damage * 3 >= this.target.currentHealth + this.target.currentBlock && !this.target.halfDead && !this.target.hasPower("Minion")) {
                    this.addToTop(new VFXAction(new FlyingOrbEffect(target.hb.cX, target.hb.cY)));
                    AbstractDungeon.player.increaseMaxHp(this.target.currentHealth, false);
                }
            }
